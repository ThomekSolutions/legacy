...
const CATALOG_FILE = path.join(ROOT, "assets", "generated-characters", "catalog.json");
...
for (const slot of catalog.renderOrder || []) {
  const slotDef = catalog.slots?.[slot];
  if (!slotDef) {
    failures.push(`Missing slot definition: ${slot}`);
    continue;
  }
  for (const item of slotDef.items || []) {
    if (item.id === "none") continue;
    for (const [anim, frames] of expectedAnims) {
      const assetPath = item.spritesheets?.[anim];
      if (!assetPath) {
        failures.push(`${slot}.${item.id} missing ${anim} spritesheet`);
        continue;
      }
      const absolutePath = path.join(ROOT, assetPath);
      if (!fs.existsSync(absolutePath)) {
        failures.push(`${slot}.${item.id}.${anim} missing file ${assetPath}`);
        continue;
      }
...
console.log("Character asset verification passed.");
